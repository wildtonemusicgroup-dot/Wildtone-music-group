...
async function dbGetTicketWithMessages(ticketId) {
    ...
    const { data: msgs, error: mErr } = await supabase.from('ticket_messages').select('*').eq('ticket_id', ticketId).order('created_at');
...
async function dbGetChatMessages(channel) {
    const { data, error } = await supabase.from('chat_messages').select('*').eq('channel', channel).order('created_at');
    if (error) console.error('dbGetChat:', error);
    return data || [];
}

async function dbGetDMMessages(dmKey) {
    const { data, error } = await supabase.from('chat_messages').select('*').eq('dm_key', dmKey).order('created_at');
...
async function dbGetTimeEntries() {
    const { data, error } = await supabase.from('time_entries').select('*').order('entry_date', { ascending: false });
